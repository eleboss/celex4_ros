...
CeleX4::CeleX4()
	: m_uiFullPicFrameTime(20), m_uiEventFrameTime(60), m_uiFEFrameTime(60), m_uiOverlapTime(0), m_uiClockRate(25), m_iTimeStartRecord(0), m_bRecording(false), m_lPlaybackFileSize(0), m_bReadDataByAPICore(false), m_bAutoAdjustBrightness(false), m_lPreT(0), m_fTSum(0), m_lCount(0), m_fDenoiseCount(0), m_fCompressCount(0), m_iDelta(100), m_fTao(0)
{
...
long CeleX4::pipeOutRawFPGAData(unsigned char *data)
{
	if (!isSensorReady())
	{
		return -1;
...
void CeleX4::pipeOutFPGAData()
{
	...
	if (dataLen > 0)
	{
		...
		if (dataLen != length)
		{
			...
			m_pDataProcessThread->m_updateSpecial = true;
...
void CeleX4::pipeOutFPGAData(long length)
{
	if (!isSensorReady())
	{
		return;
	...
	//Return the number of bytes read or ErrorCode (<0) if the read failed.
	long dataLen = m_pFrontPanel->blockPipeOut(0xa0, blockSize, length, pData);
	m_pDataProcessThread->m_updateSpecial = false;
	if (dataLen > 0)
	{
		//record sensor data
		if (m_bRecording)
